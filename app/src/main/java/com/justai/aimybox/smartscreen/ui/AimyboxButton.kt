package com.justai.aimybox.smartscreen.ui
...
internal class AimyboxButton @JvmOverloads constructor(
    ...
    fun observeEvents(aimybox: Aimybox) {
        val channel = aimybox.speechToTextEvents.openSubscription()
        launch {
            channel.consumeEach { event ->
                when (event) {
                    is SpeechToText.Event.SoundVolumeRmsChanged -> {
                        onRecordingVolumeChanged(event.rmsDb)
                    }
                    is SpeechToText.Event.RecognitionStarted -> {
                        onRecordingStarted()
                    }
                    is SpeechToText.Event.RecognitionResult, SpeechToText.Event.RecognitionCancelled, SpeechToText.Event.EmptyRecognitionResult -> {
                        onRecordingStopped()
                    }
                }
            }
